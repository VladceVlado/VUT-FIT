 ...
 */
void evaluateLayer(size_t inputSize, size_t neuronCount, const float *input, const float *weight, const float *bias, float *output);
...
{
    return (float *) _mm_malloc(numberOfElements * sizeof(float), 64);
}
...
int main(int argc, char* argv[])
{
    ...
    hid_t status = H5Dread(dataset, H5T_NATIVE_ULONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, &imageCount);
...
//####################################################################################################################//
    ...
    hid_t dset = H5Dcreate (file, "/output_data", H5T_NATIVE_FLOAT, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void simpleReadDataset(const char *datasetName, hid_t file, void *buffer)
{
    ...
    hid_t status = H5Dread(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buffer);
...
void evaluateLayer(size_t inputSize, size_t neuronCount, const float *input, const float *weight, const float *bias, float *output)
{
    ...
    for (size_t i = 0; i < neuronCount; i++) {
        ...
        // Odstraneni teto pragmy zvysi vykon, viz .txt
        #pragma omp simd reduction(+:x)
        for (size_t j = 0; j < inputSize; j++) {
            x += input[j] * weight[inputSize * i + j];
        }

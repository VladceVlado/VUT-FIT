...
object PeopleInYear {
  def main(args: Array[String]) {
    ...
    val countsDf = inputDf.groupBy().sum(columnNamesForYearsFromDf: _*)
    // dump results
    ...
    val yearsRddFromDf: RDD[(String, Int)] = inputRddFromDf.flatMap(row => row.getValuesMap(columnNamesForYearsFromDf))
    // reduce by key (year)

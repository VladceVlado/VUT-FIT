    ...
    */
    if (myid == 0) {
        ...
        while (fin.good()) {
            ...
            MPI_Send(&number, 1, MPI_INT, invar, TAG,
                     MPI_COMM_WORLD); //buffer,velikost,typ,rank prijemce,tag,komunikacni skupina
            invar++;
        }//while
        fin.close();
    }//nacteni souboru
    ...
    MPI_Recv(&mynumber, 1, MPI_INT, 0, TAG, MPI_COMM_WORLD,
    ...
    for (int j = 1; j <= halfcycles; j++) {
        ...
        if ((!(myid % 2) || myid == 0) && (myid < oddlimit)) {
            ...
            if (neighnumber > mynumber) {                                             //pokud je leveho sous cislo vetsi
                ...
                MPI_Send(&neighnumber, 1, MPI_INT, myid - 1, TAG, MPI_COMM_WORLD);   //pokud je mensi nebo stejne vratim
            //cout<<"sl: "<<myid<<endl;
        }//else if (liche)
